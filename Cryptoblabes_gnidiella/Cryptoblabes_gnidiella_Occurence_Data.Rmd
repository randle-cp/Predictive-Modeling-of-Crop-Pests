...
3a. Note the points that fall outside the home range. There are two ways to deal with this. One is to create a file that can be examined in excel. To do this, substitute a name for the file in the following code. **This is in fact, what I did, limiting the home range of the species to mainland Europe**

```{r A3a._Write-a-csv-file-for-examination-in-MS-excel}
write.csv(sdmdata, file=paste0(genus,"_",species,"_raw.csv"))
```
Remember, then to save that file in the right directory as a csv and reopen here
```{r A3b._Open-your_cleaned-csv-file}
sdmdata<- read.csv( file=paste0(genus,"_",species,"_raw.csv"), check.names = FALSE)
```



4. Revisualize your data to make sure that it is what you want.
```{r A4.-Revisualize-data}
#first lets get the extent of the data (the coordinates of the smallest box needed to encapsulate the data)
sdmdataframe<-data.frame(sdmdata)
data(wrld_simpl)
coordinates(sdmdataframe) <- ~lon+lat
crs(sdmdataframe) <- crs(wrld_simpl)
...
5. Remove points with the same latitude and longitude. Duplicated points will give to much importance to the variables where they are located.
...
7. At this point save your data as a csv, so we can access it again in the next stage. Make sure you name it something good
