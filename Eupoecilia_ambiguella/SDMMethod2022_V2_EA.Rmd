...
THIS IS A PROJECTION USING PREDICTIONS FROM THE EUROPEAN HOME RANGE
...
```{r}
sdmdata<-read.csv(file=paste0(genus,"_",species,"_EU.csv"))
...
write.csv(outdata, file=paste0(genus,"_",species,"_trainEU.csv"), row.names=FALSE)

#and then presdata_test
outdata<-data.frame(pres_test)
colnames(outdata)<-c("lon","lat")
write.csv(outdata, file=paste0(genus,"_",species,"_testEU.csv"), row.names=FALSE)
```
...
```{r}
gam <- gam(pa ~ Mean_durnal_temp_range + Temp_Seasonality + Mean_temp_warmiest_q +    
Ann_.precip + Precip_Seasonality + Precip_warmest_q + Rad_wettest_q + Rad_driest_q + Rad_warmest_q + Rad_coldest_q + Moisture_seasonality + Elev + human_impact,                      
...
PRES ABS POINTS HAVE NA PREDICTOR VALUES
```{r}
maxent()
...
```
This is a pretty conservative BRT
