...
```{r}
sdmdata <- read.csv(file = "Helicoverpa armigera.csv")
...
dist<-110000
x <- circles(pres_train_SPDF, d=110000, lonlat=TRUE)
...
```{r}
gam <- gam(pa ~ Mean_durnal_temp_range + Temp_Seasonality   +    Ann_.precip     +       Precip_driest_week   +  Precip_wettest_week +   Precip_Seasonality    + Precip_warmest_q    +   Precip_coldest_q     + 
Hghest_weekly_rad   +   Rad_seasonality    +    Rad_wettest_q   +       Rad_driest_q     +      Moisture_seasonality +  Elev    +               human_impact,
